...
# ガード
	execute unless data storage player:context this.StateMachine{current:"guard"} unless score @s HardCoolTime matches 1.. if predicate lib:input/sneak if predicate lib:flag/on_ground unless data storage player:context this.ActionBlock[{action:"guard"}] run function player_manager:fsm/request {state:"guard"}
# 回避
...
# CT
execute unless data storage player:context this.StateMachine.state_data.AttackID if score @s HardCoolTime matches 1.. run scoreboard players remove @s HardCoolTime 1
execute unless data storage player:context this.StateMachine.state_data.AttackID if score @s SoftCoolTime matches 1.. run scoreboard players remove @s SoftCoolTime 1
execute unless data storage player:context this.StateMachine.state_data.AttackID unless score @s SoftCoolTime matches 1.. run attribute @s movement_speed modifier remove player:cooltime
