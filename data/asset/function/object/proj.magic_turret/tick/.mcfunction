...
# ホーミング
	execute as @n[type=!#lib:no_living,tag=!_owner,tag=!_this,distance=..30] run tag @s add _target
...
# 発射
	function lib:interval/ {Tick:8,Offset:10}
	execute if score @s General.Tick matches 10.. if score $Interval _ matches 0 run function asset:object/proj.magic_turret/tick/shot

# 演出
	execute if score @s General.Tick matches 6 run data merge entity @s {transformation:{scale:[1.5f,1.5f,1.5f]},interpolation_duration:3,start_interpolation:0}
