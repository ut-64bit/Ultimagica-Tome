#> asset:object/proj.elf_whirl_charge/tick/
#
# Tick処理
#
...
# 定期的にダメージを与える
	function lib:interval/ {Tick:6,Offset:1}
	execute if score $Interval _ matches 0 run function asset:object/proj.elf_whirl_charge/tick/create_attack_data
# エンティティの衝突
...
# 演出
	...
	execute if score $Interval _ matches 0 run playsound entity.breeze.slide neutral @a ~ ~ ~ 0.8 1
	...
	execute if score @s General.Tick matches 4.. run function asset:object/proj.elf_whirl_charge/tick/rotate

scoreboard players reset $Interval _
