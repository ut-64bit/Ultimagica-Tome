#> asset:object/proj.thunder_spire_explosion/tick/explode
#

# ダメージを与える
	execute positioned ~-2.5 ~-0.5 ~-2.5 as @e[type=!#lib:no_living,tag=!_owner,tag=!_this,tag=!Game.Observer,tag=!Player.NoCollison,dx=4,dy=1,dz=4] unless function api:damage/is_invincible run tag @s add _hit
	...
	data modify storage api: in set value { Damage:1, Element:"thunder", School:"thunder", Tags:["spire"] }
