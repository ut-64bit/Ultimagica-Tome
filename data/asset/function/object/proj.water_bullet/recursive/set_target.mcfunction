#> asset:object/proj.water_bullet/recursive/set_target
#

# ターゲット候補に一時タグを付与する
	execute as @e[type=!#lib:no_living,tag=!_owner,tag=!_this,tag=!Game.Observer,distance=..15] unless function api:damage/is_invincible run tag @s add _target
