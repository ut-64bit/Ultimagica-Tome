...
# 発射
	...
	execute if score #t_Interval _ matches 0 run function asset:object/proj.water_rain_cloud/tick/shot/
	execute if score #t_Interval _ matches 2 run function asset:object/proj.water_rain_cloud/tick/shot/
	execute if score #t_Interval _ matches 4 run function asset:object/proj.water_rain_cloud/tick/shot/

	scoreboard players reset #t_Interval _

# 演出
	particle falling_water ^ ^ ^ 2 0 2 0 2 normal
	particle falling_water ^ ^ ^ 2 0 2 0 1 force
	particle dust{color:3481599,scale:3.0} ^ ^ ^ 2.2 0 2.2 0 1 force
	particle cloud ^ ^ ^ 2.3 0.4 2.3 0 6 force
