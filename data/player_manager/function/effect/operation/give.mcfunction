...
function player_manager:effect/definition/get.m with storage effect:context Request
execute unless data storage effect:context Definition run return 0

# 同じIDは原則1エントリ。再付与規則は定義側で選択する。
function player_manager:effect/operation/give/take_existing.m with storage effect:context Request
execute if data storage effect:context Current.ID run function player_manager:effect/operation/give/reapply.m with storage effect:context Definition
